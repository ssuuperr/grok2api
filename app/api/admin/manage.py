...
class EnableNsfwRequest(BaseModel):
    """单个 Token 开启 NSFW 请求"""
    token: str
    token_type: str


class BatchEnableNsfwRequest(BaseModel):
    """批量开启 NSFW 请求"""
    tokens: List[str]
    token_type: str
...
@router.post("/api/tokens/nsfw")
async def enable_token_nsfw(request: EnableNsfwRequest, _: bool = Depends(verify_admin_session)) -> Dict[str, Any]:
    """为单个 Token 开启 NSFW (Unhinged) 模式"""
    try:
        logger.debug(f"[Admin] 开启 NSFW: {request.token[:20]}...")
        
        from app.services.grok.nsfw import enable_nsfw
        success, msg = await enable_nsfw(request.token)
        
        if success:
            logger.info(f"[Admin] NSFW 开启成功: {request.token[:20]}...")
            return {"success": True, "message": msg}
        else:
            logger.warning(f"[Admin] NSFW 开启失败: {request.token[:20]}... - {msg}")
            return {"success": False, "message": msg}
            
    except Exception as e:
        logger.error(f"[Admin] NSFW 开启异常: {e}")
        raise HTTPException(status_code=500, detail={"error": f"开启失败: {e}", "code": "NSFW_ERROR"})


@router.post("/api/tokens/nsfw/batch")
async def batch_enable_tokens_nsfw(request: BatchEnableNsfwRequest, _: bool = Depends(verify_admin_session)) -> Dict[str, Any]:
    """批量开启 NSFW (Unhinged) 模式"""
    try:
        logger.debug(f"[Admin] 批量开启 NSFW: {len(request.tokens)} 个 Token")
        
        from app.services.grok.nsfw import batch_enable_nsfw
        result = await batch_enable_nsfw(request.tokens)
        
        logger.info(f"[Admin] 批量 NSFW 完成: 成功 {result['success']}, 失败 {result['failed']}")
        return {
            "success": True,
        ...
        logger.error(f"[Admin] 批量 NSFW 开启异常: {e}")
        raise HTTPException(status_code=500, detail={"error": f"批量开启失败: {e}", "code": "BATCH_NSFW_ERROR"})
